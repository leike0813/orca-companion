...
| IP-11 | 3.3、3.4 | 会话维护、模型配置与 Route Planning Handoff | `src/interfaces/tui/components/{model-picker,handoff-review}.tsx`、`src/bootstrap/tui-composition.ts`、`src/application/tui/view-model.ts`、`src/interfaces/tui/{app.tsx,ports.ts}` | 现有入口改接前驱真实 Controller；Handoff 加 Target 显式选择；压缩与事件投影从前驱权威读；只对确实不可用的能力保留结构化 blocker | 不新增页面；不在 TUI 内执行压缩、切换或交接 |
...
**新增**：`src/interfaces/tui/screens/{home,wizard,workspace}.tsx`、`src/interfaces/tui/components/{top-bar,transcript,composer,status-line,sidebar,session-picker,event-drawer,graph-inspector,interaction-card,command-palette,model-picker,handoff-review}.tsx`、`src/interfaces/tui/{app.tsx,state.ts,ports.ts}`、`src/interfaces/tui/input/keymap.ts`、`src/interfaces/tui/render/width.ts`、`src/application/tui/view-model.ts`、`src/interfaces/cli/argv.ts`、`src/bootstrap/{tui-entry.ts,tui-composition.ts}`、`tests/tui/*`（剩余接线新增 `host-wiring.test.ts`）。
